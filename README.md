...
- Timestamp - Время изменения статуса (в московском времени UTC+3, формат: YYYY-MM-DD HH:mm:ss)
...
**Примечание:** Все время в приложении (логи и CSV) отображается в московском часовом поясе (UTC+3).
...
src/
  ...
  └── utils/
      └── timeUtils.ts  # Утилиты для работы с московским временем (UTC+3)
```
